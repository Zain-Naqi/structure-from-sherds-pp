...
//     GeneticAssembler(const vector<Geom>& shard, list<LCSIndex>& LCS_out, int num_shards, const vector<double>& surface_scores) 
//         : shard_(shard), num_shards_(num_shards), surface_scores_all_(surface_scores) 
//     {   
...
//             if (original_count < surface_scores_all_.size()) {
//                 surface_scores_.push_back(surface_scores_all_[original_count]);
//             } else {
//                 surface_scores_.push_back(0.0);
//             }
//             original_count++;
//         }
//         BuildPairGroups();
...
//             // Calculate mean surface overlap for logging
//             double total_surface = 0.0;
//             int active_genes = 0;
//             const Chromosome& best = population_.front();
//             for (size_t g_idx = 0; g_idx < best.genes.size(); ++g_idx) {
//                 int choice = best.genes[g_idx];
//                 if (choice > 0) {
//                     size_t match_idx = pair_groups_[g_idx][choice - 1];
//                     total_surface += surface_scores_[match_idx];
//                     active_genes++;
//                 }
//             }
//             double mean_surface = active_genes > 0 ? total_surface / active_genes : 0.0;

//             cout << "[GA Gen " << generation 
//                  << "] Best fitness: " << population_.front().fitness 
//                  << "  Active genes: " << active_genes
//                  << "  Mean surface overlap: " << mean_surface << endl;
...
//             fitness += surface_scores_[match_idx] * kSurfaceOverlapWeight;
...
//     static constexpr double kMutationRate = 0.1;
...
//     static constexpr double kConnectivityReward = 2.0;
//     static constexpr double kConnectivityComponentPenalty = 10.0;
//     static constexpr int kPoseRelaxIterations = 5;
//     static constexpr double kPoseRelaxAlpha = 0.5;

//     static constexpr double kSurfaceOverlapWeight = 1.0;

//     vector<Geom> shard_;
//     vector<LCSIndex> matches_;
//     vector<double> surface_scores_;
//     vector<double> surface_scores_all_;
